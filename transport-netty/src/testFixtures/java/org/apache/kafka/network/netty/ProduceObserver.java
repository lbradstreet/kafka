 ...
 *
 * <p>Populated on the single simulation thread — no synchronization needed. Records what the
 * broker sees (per-request record counts and byte sizes, per-broker request distribution)
 * plus the on-the-wire in-flight high-water mark tracked by {@link SimNetwork}.
 */
public final class ProduceObserver {

    /** A shared no-op instance for scenarios that don't observe. */
    public static final ProduceObserver NONE = new ProduceObserver();
